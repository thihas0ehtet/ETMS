...
import 'package:etms/presentation/apply_leave/widgets/expandable_widget.dart';
...
class _LeaveViewState extends State<LeaveView> {
  ...
  getLeaveTypes() async {
    ...
    setState(() {
      ...
      typeStringList.addAll(controller.leaveTypes
          .map((element) => element.leaveTypeName.toString())
          .toList());
    });
  }
  ...
  getAllowedDates() async {
    ...
    String typeId = leaveTypeList[typeStringList.indexOf(selectedLeaveType) - 1]
        .leaveTypeID
        .toString();

    AllowedDatesData data = AllowedDatesData(
        leaveStartDate:
            DateFormat('dd/MM/yyyy').parse(_startDateController.text).dMY(),
        leaveEndDate:
            DateFormat('dd/MM/yyyy').parse(_endDateController.text).dMY(),
        unitId: '1',
        leaveTypeId: typeId,
        empSysId: sysId);
  ...
  getLeaveCarry() async {
    ...
    LeaveCarryData data = LeaveCarryData(
        empSysId: sysId, leaveTypeId: typeId, unitl: until, leaveAppId: '0');
  ...
  Widget showDatePickAlert(
      ...
      required bool checkFirst,
      required bool checkSecond,
            ...
            return Wrap(
              children: [
                ...
                if (requireHalfLeaveSelect)
                  Text(
                    'Please select half leave type',
                    style: latoRegular.copyWith(color: ColorResources.error),
  ...
  resetData() {
    ...
    if (attachmentStateKey.currentState != null) {
    ...
    return Stack(
      children: [
        SingleChildScrollView(
          ...
          children: [
            const Text("Leave Type").paddingOnly(top: 20, bottom: 10),
            Container(
              decoration: BoxDecoration(
                  color: ColorResources.white,
                  borderRadius: BorderRadius.circular(5),
                  border: Border.all(color: const Color(0xff475772))),
            ...
            if (selectedLeaveType.isNotEmpty &&
                leaveTypeList[typeStringList.indexOf(selectedLeaveType) - 1]
                        .fileUploadable ==
                    true)
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
            ...
            Row(
              ...
              children: [
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text("From Date").paddingOnly(top: 20, bottom: 10),
                    datePick(
                        controller: _startDateController,
                        checkFirst: fromCheckFirstHalf,
                        checkSecond: fromCheckSecondHalf,
                        isFromDate: true)
                  ],
                ),
                Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    const Text("To Date").paddingOnly(top: 20, bottom: 10),
                    datePick(
                        controller: _endDateController,
                        checkFirst: toCheckFirstHalf,
                        checkSecond: toCheckSecondHalf,
                  ...
                  child: ClipRRect(
                    ...
                    child: DataTable(
                      ...
                      headingRowColor: WidgetStateColor.resolveWith(
                        (states) => ColorResources.primary200,
                      ),
                      columns: const [
                        DataColumn(
                          label: Expanded(
                      ...
                      rows: List.generate(
                        ...
                        (index) => DataRow(
                          color: WidgetStateProperty.all<Color>(
                            index % 2 == 0
                                ? Colors.white
                                : ColorResources.primary50,
                          ),
                          cells: [
                            DataCell(Center(
                              child: Text(
                                DateTime.parse(dateDetailList[index]
                                        .leaveDate
                                        .toString())
                                    .dMY()
                                    .toString(),
                                style: latoRegular.copyWith(fontSize: 13),
                              ),
                            )),
                            DataCell(Center(
                              child: Text(
                                dateDetailList[index].days.toString(),
                                style: latoRegular.copyWith(fontSize: 13),
                              ),
                            )),
                            DataCell(Center(
                              child: Text(
                                dateDetailList[index].halfType.toString(),
                                style: latoRegular.copyWith(fontSize: 13),
                              ),
                            )),
                            DataCell(Center(
                              child: Text(
                                dateDetailList[index].duration.toString(),
                                style: latoRegular.copyWith(fontSize: 13),
                              ),
                            )),
                          ],
            ...
            const Text("Remark").paddingOnly(top: 20, bottom: 10),
            SimpleTextFormField(
        ...
        Align(
            ...
            child: CustomButton(
              onTap: () async {
                if (sumOfDates > 0) {
                  String typeId = leaveTypeList[
                          typeStringList.indexOf(selectedLeaveType) - 1]
                      .leaveTypeID
                      .toString();
                  List<LeaveProposalDetail> detailList = [];
                  detailList.addAll(dateList
                      .map((element) => LeaveProposalDetail(
                          leaveProposeDetailID:
                              element.leaveApplicationDetailID,
                  ...
                  ApplyLeaveData data = ApplyLeaveData(
                      leaveProposeID: 1,
                      empSysID: sysId,
                      leaveStartDate: DateFormat('yyyy-MM-ddTHH:mm:ss.SSSSSSZ')
                          .format(DateTime.parse(DateFormat('dd/MM/yyyy')
                              .parse(_startDateController.text)
                              .toString()))
                          .toString(),
                      leaveEndDate: DateFormat('yyyy-MM-ddTHH:mm:ss.SSSSSSZ')
                          .format(DateTime.parse(DateFormat('dd/MM/yyyy')
                  ...
                  if (sumOfDates > leaveCarry.balance!.toDouble()) {
                    'You have only ${leaveCarry.balance} days left for carry forward.'
                        .error();
                  } else {
                    if (selectedLeaveType.isNotEmpty &&
                        leaveTypeList[
                                    typeStringList.indexOf(selectedLeaveType) -
                                        1]
                                .fileUploadable ==
                            true) {
                      if (attachmentStateKey.currentState!.getImageFile() ==
                          null) {
                        'Please take the attachment photo'.error();
                      } else {
                        bool success = await controller.saveLeaveApplication(
                            data: data,
                            imageFile: attachmentStateKey.currentState!
                                .getImageFile());
                        if (success) {
                          resetData();
                        }
                      ...
                      bool success =
                          await controller.saveLeaveApplication(data: data);
                      if (success) {
                        resetData();
                      }
                    }
                  ...
                  'There are no available dates left for leave during the selected period.'
                      .error();
                }
              },
              text: 'Submit',
